...
class Sendler_msg:
    ...
    async def sendler_photo_call(self, call, photo, text, keyb):
        try:
            with open(photo, 'rb') as file:
                ...
                res_ = await call.message.edit_media(media=file_photo)
                res_ = await call.message.edit_caption(caption=text, reply_markup=keyb)
        except:
            try:
                with open(photo, 'rb') as file:
                    res_ = await call.message.bot.send_photo(call.message.chat.id, file, caption=(text),
                                                      reply_markup=keyb)
            except Exception as es:
    ...
    async def new_sendler_message_call(self, call, text, keyb):

        try:
            await call.message.bot.send_message(call.message.chat.id, text,
                                                reply_markup=keyb)
        except Exception as es:
            print(f'Ошибка при отправке сообщения new_sendler_message_call с фото {es}')
            return False
